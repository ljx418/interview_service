...
当前文档主线已进入 P8.1 Chatbox-first 工作台信息架构修正阶段：保留 P8 已完成的资料准备、JD 导入、岗位选择和 JD 定制简历能力，但将后续体验目标调整为“用户指导 - Chatbox - 工作台”。Chatbox 必须始终是中央第一优先展示和第一交互路径；资料/JD/简历生成入口应紧贴输入框或进入左右辅助面板，不能用大型 workflow strip 把聊天时间线挤出首屏。当前 drawio 第 4 页已要求用具体代码实体、状态颜色、上游/下游关系和禁止关系指导后续开发。P8.1 当前只做文档开发，不代表前端修复已经完成。
...
- `docs/active/01_STAGE_PRD.md` - P8.1 Chatbox-first 信息架构修正 PRD、P8-JD Intake 自动化候选 PRD、P5.5/P6/P7 自动化候选基线和历史阶段边界。
- `docs/active/02_TARGET_ARCHITECTURE.md` - P8.1 三栏 Chatbox-first 目标架构、P8 资料准备向导、JD 导入中心、JD 定制简历目标架构、当前架构差异和具体代码实体关系。
- `docs/active/03_MILESTONES_AND_DELIVERY_PLAN.md` - P8.1 文档阶段与后续开发里程碑、历史阶段里程碑和出门条件。
- `docs/active/04_ACCEPTANCE_GATES.md` - P8.1 Chatbox-first 信息架构验收门槛、P8 文档验收门槛、P7-post P5-REAL 复验条件和最终出门条件。
...
- `docs/active/jobpilot-stage-gap-and-acceptance.drawio` - P8.1 Chatbox-first 修正、目标体验、当前差距、代码实体分层交互关系、开发验收计划、里程碑、门槛和出门条件图。
