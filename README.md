...
## 当前 MVP / P11 真实市场数据 Provider Opt-in Level1 自动化候选收口
...
P8.1 Chatbox-first 自动化候选已完成并作为当前前端基线保留，但人工体验反馈表明页面仍像向导卡片堆叠，Chatbox 主体感不足，左侧求职态势和顶部服务状态也未形成清晰产品模型。P9 Chatbox-native 求职情报与申请包工作台自动化候选已完成第一轮实现：页面被重新组织为“顶部服务中心 - 左侧求职态势图 - 中央 Chatbox 主控台 - 右侧产物台”，新增本地/fixture 求职态势地图、机会匹配、投递流程、Chatbox 本地命令路由、search run 表达、项目故事草稿和 P9 中文自动化验收报告。P9 结论只覆盖本地 UI 信息架构、求职情报可视化层和现有能力重新组织；不代表全网 JD 搜索、ASR、真实 provider、MCP/Skill、招聘平台自动接入或自动投递已实现。

上一自动化候选为 P9.1 本地候选完成。P9.1 在 P9 Chatbox-native 工作台基础上，已把人工反馈中最关键的三类缺陷落地为本地可验收实现：市场地图从低保真示意图升级为 ECharts 行政区划下钻式招聘情报地图，支持全国/省/市/区县 drilldown、visualMap、tooltip、toolbox、面包屑返回、source legend 和 Chatbox 联动；真实市场数据以 `Market Provider: not_configured` 和 fixture/manual/public/opt-in source legend 明确边界，未授权不外呼；Chatbox 已增加 Socratic Intake 一问一答采集简历事实、项目故事、能力证据和不可编造边界。P9.1 不代表真实市场 provider、招聘平台抓取、真实 ASR、真实 provider、自动投递或 MCP/Skill 连通性已经完成。

P10-CLI 本地命令入口自动化候选已完成：`jobpilot` CLI 已能包装现有 FastAPI、examples/fixture、workspace、artifact 和报告能力，让人类、Codex CLI、ClaudeCode CLI 或其他本地 Agent 用稳定命令执行本地验收和查看产物；P10 中文验收报告为 `docs/reports/P10_CLI_ACCEPTANCE_REPORT.html`。该结论只覆盖本地 CLI 命令入口，不代表 MCP server、真实 provider、真实个人资料、招聘平台抓取、真实市场 provider、ASR、会议平台、自动投递或 SaaS 能力已通过。

当前主线为 P11 真实市场数据 Provider Opt-in Level1 自动化候选收口。P11 Level1 已实现本地/记录数据 market provider 边界、FastAPI market API、SQLite market schema、`JobSearchRun`、`JobMarketSnapshot`、source refs、CLI 状态输出、Chatbox 市场状态联动、P11 eval 和中文 HTML 验收报告；不默认发起真实 API 请求，不接入 BOSS/猎聘/拉勾/LinkedIn 等招聘平台，不绕风控，不自动投递。

P11 后续实现验收采用分级口径：Level 1 只代表本地实现和 fixture/recorded/public/manual 路径通过；Level 2 才代表指定真实 market provider 在用户授权和凭据配置下完成 opt-in 调用。没有合法 provider 凭据或公开调用源时，不得声明 Level 2。
...
- `docs/active/01_STAGE_PRD.md` - P11 真实市场数据 Provider Opt-in PRD、P10-CLI 自动化候选完成状态、P9.1/P9 自动化候选基线和历史阶段边界。
- `docs/active/02_TARGET_ARCHITECTURE.md` - P11 目标架构、当前架构差异、具体 provider / normalizer / snapshot / evidence 实体关系，以及 P10/P9.1/P9 历史架构基线。
- `docs/active/03_MILESTONES_AND_DELIVERY_PLAN.md` - P11 Level1 实现收口、Level2 真实 provider 待授权、P10/P9.1/P9 历史阶段里程碑和出门条件。
- `docs/active/04_ACCEPTANCE_GATES.md` - P11 Level1 验收门槛、Level2 真实 provider 待授权门槛、P10/P9.1/P9 历史门槛。
...
- `docs/active/22_P8_1_CHATBOX_FIRST_WORKSPACE_PLAN.md` - P8.1 Chatbox-first 工作台信息架构修正计划，定义“用户指导 - Chatbox - 工作台”和聊天优先验收门槛。
- `docs/active/23_P9_CHATBOX_NATIVE_JOB_INTELLIGENCE_PLAN.md` - P9 Chatbox-native 求职情报与申请包工作台计划，定义顶部服务中心、左侧求职态势图、中央 Chatbox、右侧产物台和后续开发验收边界。
- `docs/active/24_P9_1_MARKET_DATA_AND_SOCRATIC_PROTOTYPE_PLAN.md` - P9.1 本地自动化候选计划与实现记录，定义真实市场数据 opt-in provider 边界、行政区划下钻式市场地图和 Socratic Intake 资料补全路径。
- `docs/active/25_P10_CLI_LOCAL_COMMAND_ENTRY_PLAN.md` - P10-CLI 本地命令入口计划和实现收口记录，定义命令契约、代码实体、验收门槛和高风险边界。
- `docs/active/26_P11_MARKET_PROVIDER_OPT_IN_PLAN.md` - P11 真实市场数据 Provider Opt-in Level1 开发与验收计划，定义 provider 状态、授权门、数据契约、source refs、验收门槛、高风险边界和 Level2 待授权范围。
- `docs/p9_1_market_socratic_review/` - P9.1 可交给人类或 ChatGPT 审计的 PRD 扩展、行政区划下钻地图原型、provider 调研、Socratic 对话设计、目标页面与模块详细设计 HTML 审查页和审计提示词。
- `docs/active/stage-reviews/P9_1_DOCUMENTATION_DEVELOPMENT_AUDIT.md` - P9.1 历史文档开发审计，已被 P9.1-M0 到 M5 本地自动化候选实现接续。
- `docs/active/stage-reviews/P9_1_DOCUMENTATION_COVERAGE_REAUDIT.md` - P9.1 历史文档覆盖度复审，已追加 P9.1 本地自动化候选完成结论。
- `docs/active/stage-reviews/P9_1_M0_START_AUDIT.md` 到 `P9_1_M5_ACCEPTANCE_REPORT_AUDIT.md` - P9.1 本地自动化候选子阶段审计记录。
- `docs/active/stage-reviews/P9_PLUS_REMAINING_SCOPE_GATE_AUDIT.md` - P9+ 剩余开发准入审计，说明真实 provider、真实资料、MCP/CLI、ASR、会议平台、自动投递等后续能力为什么不能在缺少专项文档时直接实现。
- `docs/active/stage-reviews/P10_CLI_DOCUMENTATION_DEVELOPMENT_AUDIT.md` - P10-CLI 文档开发审计，说明本阶段只规划 CLI，不进入代码实现。
- `docs/active/stage-reviews/P10_CLI_DOCUMENTATION_COVERAGE_REAUDIT.md` - P10-CLI 文档覆盖度复审，判断是否足以支撑后续 P10-CLI 自动化开发。
- `docs/reports/P9_1_MARKET_SOCRATIC_ACCEPTANCE_REPORT.html` - P9.1 行政区划市场地图与 Socratic Intake 中文自动化验收报告，包含真实界面截图、命令证据和未验证范围。
- `docs/active/stage-reviews/P9_DOCUMENTATION_DEVELOPMENT_AUDIT.md` - P9 历史文档开发审计，记录进入代码实现前的文档阶段边界。
- `docs/active/stage-reviews/P9_DOCUMENTATION_COVERAGE_REAUDIT.md` - P9 文档覆盖度复审，判断当前文档是否完整解决用户诉求、是否落盘到 Markdown/drawio、是否足以支撑 P9-M0 到 P9-M9。
- `docs/active/stage-reviews/P9_DETAILED_DEVELOPMENT_AND_ACCEPTANCE_PLAN.md` - P9-M0 到 P9-M9 详细开发及验收计划，定义每阶段开发范围、验收标准、证据和打回条件。
- `docs/active/stage-reviews/P9_EXTERNAL_REVIEW_REVISION_AUDIT.md` - P9 外部审计意见修订记录，明确 P9 只能做 Chatbox-first UI/信息结构重构和求职情报可视化层，不能扩张成新系统平台。
- `docs/active/stage-reviews/P8_1_DOCUMENTATION_DEVELOPMENT_AUDIT.md` - P8.1 文档开发审计，记录当前只做文档修正、不进入代码实现、外部审计确认可进入 P8.1-M0 的结论。
...
- `docs/active/jobpilot-stage-gap-and-acceptance.drawio` - P9 Chatbox-native 目标架构、当前差距、代码实体分层交互关系、开发验收计划、里程碑、门槛和出门条件图。
- `docs/active/jobpilot-p9-1-market-socratic-gap.drawio` - P9.1 行政区划下钻市场地图、真实数据 provider、Socratic Intake、目标架构差异、代码实体状态、开发计划、里程碑和出门条件图。
- `docs/active/jobpilot-p10-cli-local-entry-gap.drawio` - P10-CLI 本地命令入口目标架构、当前差异、代码实体状态、命令流、开发验收计划和出门条件图。
- `docs/active/jobpilot-p11-market-provider-optin-gap.drawio` - P11 真实市场数据 Provider Opt-in 目标架构、当前差异、实体状态、命令流/数据流、开发验收计划和出门条件图。
