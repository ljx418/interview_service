...
P8.1 关系说明：P8 能力不被推翻，但当前交互主次需要修正。后续 P8.1 Chatbox-first 阶段要求把三栏体验明确为“用户指导 - Chatbox - 工作台”，中央 Chatbox 始终是第一优先展示和交互路径；资料准备、JD 导入、岗位选择和简历生成入口应紧贴输入框或进入左右辅助面板，不能用大型 workflow strip 把聊天时间线挤出首屏。P8.1 当前仅做文档开发，不代表 UI 修复已经完成。
...
- 用户能在首屏看到 Chatbox 主路径，并在输入框附近或辅助面板看到资料准备入口；
