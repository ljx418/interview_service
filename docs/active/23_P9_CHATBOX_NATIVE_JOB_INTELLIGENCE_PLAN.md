...
→ Chatbox Experience Shell (`apps/chatbox/src/main.tsx`)
  ...
  → LeftIntelligencePanel（左侧求职态势图，已实现自动化候选）
    → MarketMapView（地图/图钉/缩放/拖动，已实现自动化候选）
    → OpportunityMatchPanel（岗位机会与匹配，已实现自动化候选）
    → ApplicationPipelineView（投递流程态势，已实现自动化候选）
  → ConversationPlane（中央 Chatbox，已修改为主路径）
    → Agent State / workflow strip（当前历程状态，已改造）
    → MessageTimeline（已实现基线，需保留优先级）
    → composer tool actions（输入框工具入口，已修改）
    → handleP9Command（对话意图路由，已实现自动化候选）
  → Workbench / P9ArtifactOverview（右侧产物台，已实现自动化候选）
    → CandidateProfileSummary（已实现候选）
    → JobTargetList / ResumeGenerationPlane（已实现候选，已重排）
...
→ FastAPI Agent Service (`services/api/main.py`, `services/api/schemas.py`)
  → Provider Health / Service Health（已有基础，P9 通过 UI 状态展示）
  → Job Source / Search Runs（P9 仅用用户粘贴、fixture、已导入 JD 和本地 search run 表达）
  → Profile / Resume / Application Package routes（复用已实现能力）
→ Domain Tools
  → JobSourceConnector boundary（未新增独立平台接入服务）
  → CandidateFactGraph / StoryBank boundary（P9 以本地故事草稿和现有 profile/resume/artifact 能力覆盖自动化候选）
...
| P9-DOC-M1 | 主线文档、roadmap、追踪矩阵、drawio 同步 | README/TODO/active/drawio 均进入 P9 目标口径 |
| P9-DOC-M2 | 文档审计收口 | 无新增致命或重大规格偏差；高风险能力边界清楚 |
