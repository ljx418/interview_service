...
日期：2026-06-24
状态：本地最小连续对话基线已实现；P5 本地/mock 多轮追问自动化候选已完成；P5-REAL/P5-Freeze 冻结延期到 P7 后复验；P6+P7 已成为当前阶段规划；完整 provider-backed 自由智能聊天未完成
...
2026-06-27 更新：用户认可当前 P4 冻结体验基本 OK，并要求下一阶段纳入“无限对话聊天”和“真实 provider 聊天”。随后用户确认 P5-REAL/P5-Freeze 冻结延期到 P7 后复验，P6+P7 作为当前阶段目标。本文件继续保留 P4C/P5 本地连续对话边界，P6 的长程连续对话和真实 provider opt-in 详见 `docs/active/19_P6_PROVIDER_BACKED_LONG_CONTEXT_CHAT_PLAN.md`。
...
目标：在用户明确确认外部调用后，使用真实 provider 改善自由对话能力，并通过滚动摘要、上下文快照和 workspace 检索实现可验收的长程连续对话。这里不承诺真正无限 token 或无限成本。P6-FC 出门后进入 P7 产品化 Beta，P7 完成后再执行 P7-post P5-REAL/P5-Freeze 复验。
...
- 失败降级到本地连续对话基线。
- 长对话必须有上下文压缩策略，不能把完整历史和完整个人资料无边界发送给 provider。
