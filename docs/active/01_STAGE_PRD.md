...
当前最新阶段为 P9 Chatbox-native 求职情报与申请包工作台自动化候选收口。P8-JD Intake 与 P8.1 Chatbox-first 自动化候选已经作为实现基线保留；P9 不把既有能力写成失败，也不继续堆叠向导卡片，而是在本地自动化候选范围内完成 Chatbox-native UI 信息架构、求职态势可视化层、Chatbox 本地命令路由和现有能力重新组织。

P9 要解决的问题：

```text
当前页面仍然像多个向导和工作流卡片的组合
用户不能自然地通过 Chatbox 发起 JD 搜索、资料补全、申请包生成和流程更新
左侧求职态势没有详细表达岗位市场、目标机会和投递流程
顶部缺少 provider / ASR / MCP / Skill / 外部搜索等服务状态的快速理解
右侧产物台未形成“事实摘要 - 简历 - 故事 - 申请包 - 待确认”的持续工作区
```

P9 目标体验：

```text
用户打开本地 Chatbox-native 工作台
→ 顶部看到当前服务状态、配置状态和安全边界
→ 左侧看到可折叠、可切换、可放大拖动的求职态势图
→ 中央 Chatbox 是第一交互路径，可发起 JD 搜索、薪资城市汇总、资料补全、申请包生成和流程更新
→ Chatbox 上方显示用户历程状态：探索市场、补全资料、选择岗位、生成申请包、确认事实、准备投递
→ 右侧产物台展示候选人画像、事实摘要、简历草稿、面试故事、申请包版本、source refs、pending confirmations 和导出预检
→ 用户通过对话调整事实、项目故事、岗位目标、申请包草稿和投递流程
```

P9 必须产出的用户结果：

- 顶部服务中心：展示 provider、ASR、MCP、Skill、外部搜索、workspace、安全边界和连通性状态；未配置不得写成已连通。
- 左侧求职态势图：岗位市场态势、目标机会与匹配态势、投递流程态势三大页签；支持地图/图钉/聚合或等价地图可视化，支持放大、缩小、拖动、折叠和 Chatbox 联动。
- 中央 Chatbox 主控台：时间线、输入框、工具入口和历程状态始终优先展示，不再被大型向导卡片压住。
- JD 搜索和招聘信息汇总：允许规划合规公开源、用户粘贴源、fixture 或后续授权 connector；不得把 BOSS/猎聘/拉勾等平台自动接入写成已实现。
- 资料与故事补全：通过 Chatbox 或用户明确开启的 ASR 引导收集简历模板、项目故事、能力证据、作品链接和求职偏好。
- 多 JD 申请包：针对不同 JD 生成可追溯的简历、面试故事、求职信或申请包草稿，必须展示 source refs、pending confirmations 和版本边界。
- Chatbox 驱动更新：用户能通过对话更新申请包、项目事实、目标岗位和投递流程；不得默认自动投递或对外沟通。

P9 左侧求职态势三大板块：

| 板块 | 用户问题 | 可视化与交互 |
| --- | --- | --- |
| 岗位市场态势 | 哪些城市、薪资段、技术栈和公司类型值得关注？ | 地图/图钉/热力/聚合气泡、薪资分布、技术栈热度、城市对比；支持缩放、拖动、点击城市追问 |
| 目标机会与匹配态势 | 哪些 JD 最值得投，短板是什么？ | 多 JD 卡片、匹配度、技能差距、证据覆盖度、优先级；选择 JD 后联动 Chatbox 和右侧产物 |
| 投递流程态势 | 我投了哪些公司，每家公司到哪一步？ | 招聘流程图、颜色状态、下一步动作、时间节点、备注；通过 Chatbox 更新状态 |

P9 非目标：

- 不默认登录 BOSS、猎聘、拉勾、LinkedIn 等招聘平台；
- 不绕过验证码、反爬、账号权限或平台风控；
- 不默认自动沟通、自动投递或代表用户对外发送消息；
- 不默认调用真实 MiniMax、DeepSeek、OpenAI-compatible provider；
- 不默认开启 ASR 麦克风采集或外部语音服务；
- 不把 MCP、Skill、外部搜索、真实 provider 或 ASR 的配置展示写成真实连通性通过；
- 不读取未授权真实个人资料，不执行 workspace 删除、迁移 apply 或不可逆操作；
- 不实现 SaaS、多租户、Billing。

P9 自动化候选收口标准：

- README、TODO、PRD、目标架构、里程碑、验收门槛、追踪矩阵、roadmap、drawio 和文本镜像均进入 P9 自动化候选收口口径；
- drawio 不超过 8 页，能展示目标架构与当前架构差异、开发及验收计划、项目里程碑、验收门槛和出门条件；
- 架构图必须使用具体代码实体、状态颜色、分层关系和强关联交互，不得只列抽象功能；
- 文档和报告必须把 P9 已实现范围限定为本地 UI 信息架构、可视化层、本地命令路由和现有能力重新组织；
- 文档不得把全网 JD 搜索、招聘平台自动接入、真实 ASR、真实 provider、MCP/Skill 连通性或自动投递写成已通过。

P9 实施范围硬约束：

```text
P9 = Chatbox-first UI 与信息结构重构 + 求职情报可视化层
P9 ≠ 新招聘平台系统 / 搜索系统 / 语音系统 / MCP 平台 / 自动投递系统
```
...
- UI 重排和信息架构重构；
- 现有 API 的组合与前端编排；
- Artifact、profile、job、resume、story、application package 的展示增强；
- Chatbox 交互增强和本地 workspace 状态更新；
- fixture、用户粘贴、已有本地示例或合规公开样例数据的可视化。
...
- 新增长期运行任务系统；
- 新增独立业务服务；
- 触发真实招聘平台登录、抓取、绕风控、自动沟通或自动投递；
- 默认启用真实 ASR、真实 provider、MCP/Skill 连通或读取真实个人资料。

## -5. 上一自动化候选阶段：P8.1 Chatbox-first 工作台信息架构修正

P8.1 Chatbox-first 工作台信息架构修正已经完成自动化候选，并作为 P9 的前端基线保留。P8-JD Intake 与简历生成体验强化已经完成本地/mock + 受控真实感数据自动化候选；P8.1 不推翻 P8 的资料准备、JD 导入、岗位选择和 JD 定制简历能力，而是修正界面的主次关系。
...
## -4. 历史自动化候选阶段：P8-JD Intake 与简历生成体验强化
