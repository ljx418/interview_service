...
## -6. 当前文档阶段：P9 Chatbox-native 求职情报与申请包工作台

当前最新文档阶段为 P9 Chatbox-native 求职情报与申请包工作台。P8-JD Intake 与 P8.1 Chatbox-first 自动化候选已经作为实现基线保留；P9 不把既有能力写成失败，也不继续堆叠向导卡片，而是重新定义下一阶段面向用户的核心体验。
...
- README、TODO、PRD、目标架构、里程碑、验收门槛、追踪矩阵、roadmap、drawio 和文本镜像均进入 P9 口径；
- drawio 不超过 8 页，能展示目标架构与当前架构差异、开发及验收计划、项目里程碑、验收门槛和出门条件；
- 架构图必须使用具体代码实体、状态颜色、分层关系和强关联交互，不得只列抽象功能；
- 文档不得把 P9 目标写成 P9 已实现；
- 文档不得把全网 JD 搜索、招聘平台自动接入、ASR、真实 provider、MCP/Skill 连通性或自动投递写成已通过。
