...
- P3 已完成本地自动化验收，但人工审查未完全认可当前用户体验；
- P4 当前目标是 UX 体验强化：首屏任务入口、对话反馈、推进台层级、产物卡语言、provider 语义、响应式精修、全尺寸桌面工作台体验和人工体验审查包；
- 真实 API Key、真实外部模型调用、真实个人资料、不可逆迁移仍必须暂停找用户确认。

P4 可以由自动化 Agent 直接推进的范围：

- active docs、README、TODO、drawio 和文本镜像同步；
- Gemini 前端页面审查包和静态原型；
- Chatbox 信息架构、文案、布局、状态反馈和响应式 CSS 修正；
- 任务入口、空状态、缺资料、错误恢复和 loading/progress 状态；
- artifact 摘要、确认项、版本、导出入口的可读性修正；
- provider 状态表达，不改变外部调用默认策略；
- 1200px、1440px、1600px、1920px 全尺寸桌面布局修正，前提是不引入大规模前端框架重写；
- Chrome 截图脚本隔离或 viewport emulation 清理，避免污染人工审查者浏览器；
- 使用 examples 真实感数据做端到端验收；
- Chrome 1200/1440/1600/1920/720/390 可见截图验收；
- P4 HTML 报告、PRD 规格检视、stage review；
- eval 增量补强、pytest、frontend build。

P4 必须先写计划和审计意见的范围：

- 全量前端重构或引入新的 UI framework；
- 新增复杂结构化编辑表单；
- 新增多示例数据集或真实上传复杂清洗流程；
- 重大路由/API 重构；
- 数据库 schema 不可逆迁移；
...
如果涉及浏览器体验，必须采集 Chrome 1200/1440/1600/1920/720/390 截图，并在报告中说明已验证、未验证、截图脚本是否清理 viewport emulation、以及人工体验审查范围。

以下 P3 自动化范围作为已完成基线和历史背景保留；不代表当前 P4 执行口径。
...
以下 P2 自动化范围作为已完成基线和历史背景保留；不代表当前 P4 执行口径。
...
以下能力曾作为 P2 之后的候选，但在当前 P4 中仍不进入出门条件；它们只能作为 P5+ 或独立阶段规划：
